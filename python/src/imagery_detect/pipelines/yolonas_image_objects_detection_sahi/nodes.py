...
def yolox_image_demo_sahi(img):
    ...
    result = get_sliced_prediction(
        # "C:\\xampp\\htdocs\\webmap\\python\\imagery-detect\\data\\01_raw\\raw_imagery.jpg",
        img_numpy,
        detection_model,
        slice_height=512,
        slice_width=512,
...
def yolonas_all_classes_sahi(yolonas_result_sahi):
    yolonas_classes=txt_to_json_sahi(yolonas_result_sahi)

    yolonas_classes_sahi = json.dumps(yolonas_classes, indent=4)
    file_path = "C:\\xampp\htdocs\webmap\python\imagery-detect\data\\05_model_input\yolox_classes_sahi.json"
    # Write the data to the JSON file
    with open(file_path, "w") as json_file:
        json.dump(yolonas_classes_sahi, json_file, indent=4)
    return yolonas_classes_sahi;

def save_result_yolonas_sahi(yolonas_classes_sahi, raw_coords,img):
    ...
    geojson_output_file = generate_geojson(top, left, bottom, right, yolonas_classes_sahi,img,output_file)
